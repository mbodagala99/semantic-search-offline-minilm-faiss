...
def load_queries_from_json_files(queries_dir="test_queries"):
    ...
    queries_path = current_dir / queries_dir
    if not queries_path.exists():
        raise FileNotFoundError(f"Queries directory {queries_path} not found")
...
def test_healthcare_query_routing(query_router=None, save_results=True):
    ...
    for i, test_case in enumerate(test_queries, 1):
        ...
        is_negative_scenario = expected_index == "NO_MATCH"
        ...
        try:
            ...
            if is_negative_scenario:
                ...
                if confidence_score >= query_router.confidence_threshold:
                    # False positive - negative scenario matched
                    false_positives += 1
                    status_icon = "❌"
                    status_text = "FALSE POSITIVE - Should not match"
                else:
                    # True negative - negative scenario correctly rejected
                    true_negatives += 1
                    status_icon = "✅"
                    status_text = "TRUE NEGATIVE - Correctly rejected"
            else:
                # Positive scenario
                if confidence_score >= query_router.confidence_threshold:
            ...
            result = {
                ...
                "is_negative_scenario": is_negative_scenario,
    ...
    positive_queries = total_queries - negative_scenarios
    success_rate = (high_confidence_matches / positive_queries) * 100 if positive_queries > 0 else 0
    false_positive_rate = (false_positives / negative_scenarios) * 100 if negative_scenarios > 0 else 0
    true_negative_rate = (true_negatives / negative_scenarios) * 100 if negative_scenarios > 0 else 0
    
    # Print summary
    print(f"\n📊 Test Results Summary:")
    print("-" * 50)
    print(f"   Total Queries Tested: {total_queries}")
    print(f"   Positive Scenarios: {positive_queries}")
    print(f"   Negative Scenarios: {negative_scenarios}")
    print(f"   High Confidence Matches: {high_confidence_matches}")
    print(f"   Low Confidence (Clarification): {low_confidence_clarification}")
    print(f"   Success Rate (Positive): {success_rate:.1f}%")
    print(f"   False Positives: {false_positives}")
    print(f"   True Negatives: {true_negatives}")
    print(f"   False Positive Rate: {false_positive_rate:.1f}%")
    print(f"   True Negative Rate: {true_negative_rate:.1f}%")
